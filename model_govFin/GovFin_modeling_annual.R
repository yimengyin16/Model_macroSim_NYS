# This script explores the relationship between economic variables and 
#  state gov tax revenue and conducts exploratory modeling 
# Analysis for 
#    1. National level, annual
#    2. NYS level, annual
...
# To-do list for GDP and tax revenue section

# What to model:
# 1. Personal income tax: relationship with GDP growth and returns (stock and/or portfolio)
# 2. General Sales tax: relationship with GDP, how the relationship changes during recessions
# 3. Other taxes (non-PIT-non-sales taxes): relationship with GDP
...
#    add the assumed inflation rate to the GDP growth rate. So we do not model inflation 
#    in this version. 
...
# 1. National GDP: 
#			- from FRED, 2nd quarter data for annual data. This may be more consistent with the timing of FY for most states
# 2. State GDP: 
#     - from BEA, concatenated growth series of NAICS and SIC
# 3. National and state tax revenue: 
#     - US Census Bureau Annual Survey of State and Local Government Finances.  



#' Notes on decomposing GDP and tax revenue
#'  - For now, GDP and tax are NOT decomposed into trend and cyclical components
#'  - May consider doing so later, because this will allow for more precise control=
#'    of the composition of tax revenue in the long-term. 


# Other relationships to examine:
  # 2. GDP growth and total tax revenue growth
  # 3. GDP growth and personal income tax growth
  # 4. GDP growth and sales tax growth
  # 5. GDP growth and property tax growth
  # 6. GDP growth and corporate income tax growth
  # 7. GDP growth and growth of total tax revenue minus PIT and sales
...
#    modeld using real terms. But note that pension contributions are modeled in nominal terms, which 
...
# 2019/10: Possible detrending method: Hamilton(2017) 'Why You Should Never Use the Hodrick-Prescott Filter'
...
dir_inputs_raw  <- "inputs/data_raw/"
...
# 3. Data frame of real values based on GDPCTPI/GDPinflator
# 4. Data frame of nominal values from databases



## 1. Loading saved data 
ls_financial <- readRDS(paste0(dir_inputs, "dataProc_financial.rds"))
ls_govRev    <- readRDS(paste0(dir_inputs, "dataProc_govRev.rds"))
ls_GSP       <- readRDS(paste0(dir_inputs, "dataProc_GSP.rds"))
...
df_USGDP <- 
  ls_financial$df_financial_y %>% 
	select(year, month, yearMon, GDP_mon6, GDP_avg, GDPCTPI, GDPdeflator) %>% 
	mutate(GDP_mon6 = GDP_mon6 * 1e6, # original value in $billion, turn to $thousand
				 GDP_avg  = GDP_avg  * 1e6)
#' Variables selected:
#'  - GDP_mon6: Quarterly GDP of the 3rd quarter, annualized value
#'  - GDP_avg:  Average of all quarterly GDP in a calendar year, annualized value
#'  - GDPCTPI:  GDP chain-type price index, from FRED.
#'  - GDPdeflator: GDP deflator, from FRED. Very close to GDP chain-type price index.

#' GDPCTPI will be used to adjust tax revenue


# National and state level tax revenue
taxVars <- c(
	"tax_tot_real_state",         "tax_tot_nom_state",
	"tax_indivIncome_real_state", "tax_indivIncome_nom_state",
	"tax_sales_tot_real_state",   "tax_sales_tot_nom_state",
	"tax_sales_gen_real_state",   "tax_sales_gen_nom_state",
	"tax_sales_sel_real_state",   "tax_sales_sel_nom_state",
	"tax_corpIncome_real_state",  "tax_corpIncome_nom_state"
)
...
df_GSP <- 
	ls_GSP$df_GSP_y %>% 
	select(state_abb, year, RGSP_SIC, RGSP_NAICS, NGSP_SIC, NGSP_NAICS)
	

# State level coincidence index, monthly to annual using q3 and average
df_cIdx <- 
  ls_GSP$df_GSP_q %>% 
	select(state_abb, year, qtr, cIdx) %>%
	group_by(state_abb, year) %>% 
	summarize(cIdx_qtr3 = cIdx[qtr == 3],
						cIdx_avg  = mean(cIdx),
						.groups = "drop")
...
# Time frame is based on revenue data: from 1977 to 2015 

# merging data
df_revGDP_real <- 
  df_govRev %>% 
	select(state_abb, year, contains("_nom_")) %>% 
	left_join(df_GSP %>% 
							select(state_abb, year, NGSP_SIC, NGSP_NAICS), 
						by = c("state_abb", "year")) %>% 
	left_join(df_USGDP %>% 
							select(year, GDPCTPI),
						by = "year")

# get real values using CTPI
df_revGDP_real %<>% 
	mutate(across(c(-state_abb, -year), ~.x / (GDPCTPI/100))) %>% 
	rename_with(~str_remove(.x, "_nom_state")) %>% 
	rename_with(~str_replace(.x, "NGSP", "GSP")) 

# adding national GDP and state level coincidence index
df_revGDP_real %<>% 
	left_join(df_USGDP %>% 
							select(year, GDP_mon6, GDP_avg),
						by = "year") %>% 
	left_join(df_cIdx %>% 
							select(state_abb, year, cIdx_qtr3, cIdx_avg),
						by = c("state_abb", "year"))
	

# Create annual growth rates (log difference)
df_revGDP_dlreal <-
	df_revGDP_real %>% 
	group_by(state_abb) %>% 
...
# adding national GDP and state level coincidence index, these are real values
df_revGDP_nom %<>% 
	left_join(df_USGDP %>% 
							select(year, GDP_mon6, GDP_avg),
						by = "year") %>% 
...
df_SBBI <- 
	...
	select(year, month, yearMon, 
				 #GDP_mon6,        # National GDP from FRED, in billions,
				 #GDP_avg,         # National GDP from FRED, in billions
				 #CPIU_NA,         # CPI, urban resident, not seasonally adjusted. FRED
...
# 1.1 Compute asset return from indexes
df_dlSBBI <- df_SBBI %>% 
	select(-LTGBond_Yield) %>% 
	mutate(across(c(-year, -month, -yearMon), ~log(.x/lag(.x)))) %>% 
	rename_with( ~paste0("dl", .x), c(-year, -month, -yearMon))
...
df_returns <- 
	df_SBBI %>%  
	left_join(df_dlSBBI, by = c("year", "month", "yearMon")) %>% 
	left_join(rlzCapGains_nom, by = "year") %>% 
	left_join(rlzCapGains_nom_rev, by = "year")



## 4. getting real variables 
df_returns %<>% 
	mutate(# real changes based on CPI
		...
		dlLCapStock_TRI_real      = dlLCapStock_TRI    - dlGDPCTPI,
		dlLCapStock_CAI_real      = dlLCapStock_CAI    - dlGDPCTPI,
		dlCBond_TRI_real          = dlCBond_TRI        - dlGDPCTPI,    
		dlLTGBond_TRI_real        = dlLTGBond_TRI      - dlGDPCTPI,  
		dlMTGBond_TRI_real        = dlMTGBond_TRI      - dlGDPCTPI,
		# SP500_return_nyu_real2     = SP500_return_nyu   - dlGDPCTPI, 
		# TBill_return_nyu_real2     = TBill_return_nyu   - dlGDPCTPI, 
		# TBond_return_nyu_real2     = TBond_return_nyu   - dlGDPCTPI,
		capgains_chg_real         = capgains_chg       - dlGDPCTPI,
		tax_gains_chg_real        = tax_gains_chg      - dlGDPCTPI,
		...
		LCapStock_TRI_real      = LCapStock_TRI  /(GDPCTPI/100),
		LCapStock_CAI_real      = LCapStock_CAI  /(GDPCTPI/100),
		CBond_TRI_real          = CBond_TRI      /(GDPCTPI/100),    
		LTGBond_TRI_real        = LTGBond_TRI    /(GDPCTPI/100),  
		MTGBond_TRI_real        = MTGBond_TRI    /(GDPCTPI/100),
		capgains_real           = capgains   /(GDPCTPI/100),
		tax_gains_real          = tax_gains  /(GDPCTPI/100)
	) %>% 
	select(-GDPCTPI) %>% 
	ungroup
